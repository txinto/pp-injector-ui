...
PPInjectorUI_dre_t PPInjectorUI_dre = {
    .enabled = true,
...
extern void PPInjectorUI_comms_set_tx_callback_bridge(void (*cb)(const char *, void *), void *ctx);
...
static void PPInjectorUI_uart_init_once(void)
{
    ...
    ESP_ERROR_CHECK(uart_set_pin(
        port,
        CONFIG_PPINJECTORUI_UART_TX_GPIO,
    ...
    ESP_ERROR_CHECK(uart_driver_install(
        port,
        CONFIG_PPINJECTORUI_UART_RX_BUF_SIZE,
        CONFIG_PPINJECTORUI_UART_TX_BUF_SIZE,
        0,
        NULL,
        0
    ));
    ...
    ESP_LOGI(
        TAG,
        "UART ready: uart=%d tx=%d rx=%d baud=%d",
        port,
        CONFIG_PPINJECTORUI_UART_TX_GPIO,
        CONFIG_PPINJECTORUI_UART_RX_GPIO,
...
PPInjectorUI_return_code_t PPInjectorUI_start(void)
{
    ...
    BaseType_t ok = xTaskCreatePinnedToCore(
        PPInjectorUI_task,
        "PPInjectorUI",
        CONFIG_PPINJECTORUI_TASK_STACK,
        NULL,
        CONFIG_PPINJECTORUI_TASK_PRIO,
        &s_task,
        core
    );
    if (ok != pdPASS) {
...
{
    if (!s_task) return PPInjectorUI_ret_ok;
    s_run = false;
...
{
    if (!dst) return PPInjectorUI_ret_error;
    _lock();
...
PPInjectorUI_return_code_t PPInjectorUI_spin(void)
{
    ...
    if ((now_ms - s_spin_log_last_ms) >= 1000U) {
        ...
        ESP_LOGD(TAG, "ppinjector spin done");
    }
