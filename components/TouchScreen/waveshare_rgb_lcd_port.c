...
#define BOOT_RGB_BOUNCE_BUFFER_SIZE_PX                                         \
  (LCD_H_RES * BOOT_RGB_BOUNCE_BUFFER_LINES)
...
static void panel_backlight_enable(void) {
  ...
  gpio_config(&io_conf);
  gpio_set_level(CONFIG_TOUCHSCREEN_BACKLIGHT_GPIO,
                 CONFIG_TOUCHSCREEN_BACKLIGHT_ON_LEVEL);
#endif
#endif
}
...
esp_err_t waveshare_esp32_s3_rgb_lcd_init() {
  ...
  if (!s_panel_initialized) {
    ...
    ESP_LOGI(TAG, "Initialize RGB LCD panel"); // Log the initialization of the
                                               // RGB LCD panel
    ESP_ERROR_CHECK(
        esp_lcd_panel_init(panel_handle)); // Initialize the LCD panel
      ...
      .flags =
          {
              ...
              .mirror_x = 0, // Rotate 180º: mirror X
              .mirror_y = 0, // Rotate 180º: mirror Y
          },
  };
  ESP_ERROR_CHECK(esp_lcd_touch_new_i2c_gt911(
      tp_io_handle, &tp_cfg,
      &tp_handle)); // Create new I2C GT911 touch controller
#endif              // CONFIG_LCD_TOUCH_CONTROLLER_GT911

  ESP_ERROR_CHECK(lvgl_port_init(
      panel_handle,
      tp_handle)); // Initialize LVGL with the panel and touch handles
  /* Reset display mirror to upright */
  ...
  esp_lcd_rgb_panel_event_callbacks_t cbs = {
      ...
      .on_bounce_frame_finish =
          rgb_lcd_on_vsync_event, // Callback for bounce frame finish
#else
...
esp_err_t waveshare_esp32_s3_rgb_lcd_init_nolvgl() {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "boot display: esp_lcd_new_rgb_panel failed: %s",
             esp_err_to_name(err));
    return err;
  }
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "boot display: esp_lcd_panel_init failed: %s",
             esp_err_to_name(err));
    return err;
  }
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "boot display: esp_lcd_panel_mirror failed: %s",
             esp_err_to_name(err));
    return err;
  }
...
static esp_err_t lcd_draw_hline(int x, int y, int w, uint16_t color) {
  ...
  if (row_cap < w) {
    uint16_t *new_row = (uint16_t *)heap_caps_realloc(row, sizeof(uint16_t) * w,
                                                      MALLOC_CAP_8BIT);
    if (!new_row) {
...
esp_err_t waveshare_esp32_s3_rgb_lcd_fill_rect(int x, int y, int w, int h,
                                               uint16_t color) {
  if (!s_panel_initialized || !s_panel_handle) {
    return ESP_ERR_INVALID_STATE;
  }
...
esp_err_t waveshare_esp32_s3_rgb_lcd_clear(uint16_t color) {
  return waveshare_esp32_s3_rgb_lcd_fill_rect(0, 0, LCD_H_RES, LCD_V_RES,
                                              color);
}
...
static const uint8_t *font5x7(char c) {
  ...
  case ':':
    return COLON;
  case '/':
    return SLASH; case '%': return PERCENT;
  case '0':
    return ZERO;
  case '1':
    return ONE;
...
esp_err_t waveshare_esp32_s3_rgb_lcd_draw_center_text(const char *text,
                                                      uint16_t fg,
                                                      uint16_t bg) {
  if (!text)
    ...
    for (int j = 0; j < line_len; j++) {
      ...
      for (int r = 0; r < 7; r++) {
        ...
        for (int col = 0; col < 5; col++) {
          if (row & (1 << (4 - col))) {
            err = waveshare_esp32_s3_rgb_lcd_fill_rect(
                gx + col * scale, y_line + r * scale, scale, scale, fg);
            if (err != ESP_OK)
              return err;
          }
        }
...
int waveshare_esp32_s3_rgb_lcd_width(void) { return LCD_H_RES; }

int waveshare_esp32_s3_rgb_lcd_height(void) { return LCD_V_RES; }
...
void example_lvgl_demo_ui() {
  // Reserved for optional manual demos. Not used by TouchScreen component
  // runtime.
}
