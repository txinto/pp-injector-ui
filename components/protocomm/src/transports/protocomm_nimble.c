...
/*****************************************************************************************/
// BEGIN PATCH
// This is for calling simple_ble_advertise externally
...
// BEGIN PATCH
// This is for modifying the advertsiement data to add manufacturing one
// TODO: Try to remove this patch
...
esp_err_t simple_ble_set_mfg_data(uint8_t *mfg_data, ssize_t mfg_data_len)
{
    ESP_LOGW(TAG,"mfg_data_len %d ", adv_data.mfg_data_len);
    ESP_LOGW(TAG,"mfg_data_len %d ", mfg_data_len);    
    if (!mfg_data || !mfg_data_len) {
        return ESP_ERR_INVALID_ARG;
    }

    adv_data.mfg_data = (uint8_t *) malloc(mfg_data_len);
    if (adv_data.mfg_data == NULL) {
        ESP_LOGE(TAG, "Error allocating memory for mfg_data");
        return ESP_ERR_NO_MEM;
    }

    adv_data.mfg_data_len = mfg_data_len;
    memcpy(adv_data.mfg_data, mfg_data, mfg_data_len);
    return ESP_OK;
}
// END PATCH
